...
void char_24x16(uint16_t aDig, uint16_t aX, uint16_t aY);
...
//==============================================================================
void Gotoxy(char x,char y){
		lcd_write(FLAG_CMD,(0xB0|(y&0x0F)));		 // Y axis initialisation: 0100 yyyy			
		lcd_write(FLAG_CMD,(0x00|(x&0x0F)));		 // X axis initialisation: 0000 xxxx ( x3 x2 x1 x0)
		lcd_write(FLAG_CMD,(0x10|((x>>4)&0x07))); // X axis initialisation: 0010 0xxx  ( x6 x5 x4)
}
//==============================================================================
...
void char_24x16(uint16_t aDig, uint16_t aX, uint16_t aY)
{
  ...
  for(i = 0; i < SYM_HEIGHT; i++)
  {
    ...
    for(j = 0; j < SYM_WIDGHT; j++)
      lcd_write(FLAG_DATA, Times_16x24[j+i*SYM_WIDGHT + aDig*SYM_WIDGHT*SYM_HEIGHT]);
  }
}
//==============================================================================
void char_10x16(uint16_t aX, uint16_t aY,uint16_t aDig)
{
	int i = 0, j = 0;
  for(i = 0; i < 2; i++)
  {
    Gotoxy(aX,aY+i);    
    for(j = 0; j < 10; j++) lcd_write(FLAG_DATA, courier_10x16[j+i*10 + aDig*10*2]);
  }
}
//==============================================================================
